...
router.get('/packages', auth_1.requireAuth, (_req, res) => {
    res.json({ packages: PACKAGES });
});
// POST /api/telecom/internet
router.post('/internet', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const wallet = await client.query(`SELECT rial_balance FROM wallets WHERE user_id = $1 FOR UPDATE`, [req.userId]);
        ...
        await client.query(`UPDATE wallets SET rial_balance = rial_balance - $1, updated_at = NOW() WHERE user_id = $2`, [pkg.price_rial, req.userId]);
        await client.query(`INSERT INTO telecom_orders (user_id, target_phone, operator, package_id, order_type, amount_rial, status, ref_code)
       VALUES ($1,$2,$3,$4,'internet',$5,'completed',$6)`, [req.userId, phone, pkg.op, packageId, pkg.price_rial, result.refCode]);
        await client.query(`INSERT INTO transactions (user_id, type, direction, currency, amount, status, notes)
       VALUES ($1,'telecom_internet','out','rial',$2,'completed',$3)`, [req.userId, pkg.price_rial, `${pkg.name} for ${phone}`]);
...
router.post('/charge', auth_1.requireAuth, async (req, res) => {
    const { phone, packageId } = req.body;
    if (!phone || !packageId) {
        res.status(400).json({ error: 'missing_fields' });
        return;
    ...
    try {
        ...
        await client.query(`UPDATE wallets SET rial_balance = rial_balance - $1, updated_at = NOW() WHERE user_id = $2`, [pkg.price_rial, req.userId]);
        await client.query(`INSERT INTO telecom_orders (user_id, target_phone, operator, package_id, order_type, amount_rial, status, ref_code)
       VALUES ($1,$2,$3,$4,'charge',$5,'completed',$6)`, [req.userId, phone, pkg.op, packageId, pkg.price_rial, result.refCode]);
        await client.query(`INSERT INTO transactions (user_id, type, direction, currency, amount, status, notes)
       VALUES ($1,'telecom_charge','out','rial',$2,'completed',$3)`, [req.userId, pkg.price_rial, `شارژ ${pkg.name} for ${phone}`]);
        await client.query('COMMIT');
        res.json({ success: true, refCode: result.refCode });
    }
    catch (err) {
