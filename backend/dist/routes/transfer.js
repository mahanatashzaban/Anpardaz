...
router.post('/', auth_1.requireAuth, async (req, res) => {
    const { srcCurrency, destType, destValue, amount, destCurrency, paymentId } = req.body;
    ...
    try {
        ...
        const walletResult = await client.query(`SELECT rial_balance, usdt_balance FROM wallets WHERE user_id = $1 FOR UPDATE`, [req.userId]);
        ...
        const priceRow = await client.query(`SELECT price FROM price_cache WHERE symbol = 'usdt_rial' ORDER BY fetched_at DESC LIMIT 1`);
        ...
        // Deduct from sender
        if (deductRial > 0) {
            await client.query(`UPDATE wallets SET rial_balance = rial_balance - $1, updated_at = NOW() WHERE user_id = $2`, [Math.round(deductRial), req.userId]);
        }
        if (deductUsdt > 0) {
            await client.query(`UPDATE wallets SET usdt_balance = usdt_balance - $1, updated_at = NOW() WHERE user_id = $2`, [deductUsdt, req.userId]);
        }
        // Credit receiver if Kelid-to-Kelid
        let counterpartUserId = null;
        if (destType === 'kelid') {
            const receiverResult = await client.query(`SELECT u.id FROM users u WHERE u.phone = $1 AND u.is_active = true`, [destValue]);
            if (receiverResult.rowCount && receiverResult.rowCount > 0) {
                ...
                if (destCurrency === 'rial' && destAmount > 0) {
                    await client.query(`UPDATE wallets SET rial_balance = rial_balance + $1, updated_at = NOW() WHERE user_id = $2`, [Math.round(destAmount), counterpartUserId]);
                }
                if (destCurrency === 'usdt' && destAmount > 0) {
                    await client.query(`UPDATE wallets SET usdt_balance = usdt_balance + $1, updated_at = NOW() WHERE user_id = $2`, [destAmount, counterpartUserId]);
                }
            }
        }
...
router.get('/lookup-user', auth_1.requireAuth, async (req, res) => {
    const { phone } = req.query;
    if (!phone) {
        res.status(400).json({ error: 'phone_required' });
        return;
    }
    const result = await db_1.db.query(`SELECT id, kelid_id FROM users WHERE phone = $1 AND is_active = true`, [phone]);
